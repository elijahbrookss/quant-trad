...
1. **Local durable spool on block storage.** A fenced WebSocket session assigns
   receive ordinals and fsyncs exact frames before canonical publication. The
   spool is bounded, recoverable staging, not long-term truth.
2. **Immutable raw archive in object storage.** Sealed Parquet/ZSTD objects
   ...
   manifests are inserted only after upload checksum verification.
3. **Typed hot canonical/derived facts in PostgreSQL/Timescale.** Trades, L2
   snapshots/mutations, validity, BBO, depth, aggregates, relationships,
   quality, and feature revisions use typed tables, append-only revisions, the
   shared fact clock, provenance, and known-at semantics. Mutable book state is
