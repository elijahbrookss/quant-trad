...
Candles and OI use append-only revisions and one shared market commit clock.
...
Funding, basis, or another observation must define its own fact contract and
...
collector handler supports only Coinbase OI; it does not imply historical OI,
aggregation, funding, or basis support.
