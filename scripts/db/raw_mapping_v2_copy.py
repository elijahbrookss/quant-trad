...
def _key_parameters(keys):
    # Parallel unnest preserves pairs; independent ANY/IN lists do not.
    return {"raw_keys": [key[0] for key in keys],
            "manifest_keys": [key[1] for key in keys]}


def _rows_for_keys(conn, relation, keys):
    # relation is one of this module's fixed source/target names. A paired join
    # avoids the large OR plan generated by thousands of composite IN values.
    return [dict(row) for row in conn.execute(text(f"""
        SELECT original.* FROM {relation} AS original JOIN {_KEY_ROWS}
          ON original.raw_record_id=wanted.raw_record_id
         AND original.manifest_id=wanted.manifest_id
    """), _key_parameters(keys)).mappings()]
...
def copy_page(conn, *, page_rows=128, timeout_seconds=30):
    ...
    with _step(conn,timeout_seconds):
        ...
        if not state["baseline_complete"]:
            ...
            """),{"limit":page_rows}).all()
            rows=_rows_for_keys(conn,SOURCE,queued) if queued else []
        ...
        if rows:
            ...
            conn.execute(text(f"""
                DELETE FROM {QUEUE} AS queued USING {_KEY_ROWS}
                WHERE queued.raw_record_id=wanted.raw_record_id
                  AND queued.manifest_id=wanted.manifest_id
            """),_key_parameters(copied_keys))
        if not state["baseline_complete"]:
            if rows:
