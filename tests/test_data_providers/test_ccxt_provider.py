...
def patch_binanceus(monkeypatch):
    ...
    DummyExchange.max_per_call = 600
    DummyExchange.step_ms = 15 * 60 * 1000
    DummyExchange.until_ms = None
...
def test_fetch_from_api_continues_when_exchange_caps_limit():
    provider = CCXTProvider('binanceus')

    start = dt.datetime(2024, 8, 1, tzinfo=dt.timezone.utc)
    end = start + dt.timedelta(days=90)

    # Simulate Binance US clamping the batch size to 1,000 candles regardless of request size.
    DummyExchange.max_per_call = 1000
    DummyExchange.until_ms = int(end.timestamp() * 1000)
    ...
    assert isinstance(frame, pd.DataFrame)
    assert not frame.empty
    assert frame['timestamp'].max() >= pd.Timestamp(end) - pd.Timedelta(minutes=15)
