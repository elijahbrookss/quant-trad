...
def test_emit_runtime_event_keeps_runtime_events_in_memory_only() -> None:
    ...
    assert runtime._run_context.runtime_event_stream[0]["context"]["series_key"] == "instrument-btc|1m"


def test_signal_emitted_context_requires_series_key() -> None:
    bar_ts = datetime(2026, 1, 1, tzinfo=timezone.utc)

    with pytest.raises(ValueError, match="context.series_key is required for SIGNAL_EMITTED"):
        SignalEmittedContext(
            run_id="run-1",
            bot_id="bot-1",
            strategy_id="strategy-1",
...
def test_commit_botlens_fact_payload_allocates_seq_before_run_context_exists() -> None:
    ...
    assert outcome.batch.live_payload["seq"] == 1
    assert outcome.batch.live_payload["facts"][0]["fact_type"] == "candle_upserted"
    assert appended_batches == []


def test_commit_botlens_fact_payload_uses_async_batch_dependency_when_available() -> None:
    appended_batches = []

    def _append_batch(**_kwargs):
        raise AssertionError("single canonical append should not be used when batch appender is configured")

    def _append_batches(items):
        appended_batches.append([dict(item) for item in items])
    ...
    deps = BotRuntimeDeps(
        **{
            **deps.__dict__,
            "append_botlens_canonical_fact_batch": _append_batch,
            "append_botlens_canonical_fact_batches": _append_batches,
        }
    )
    runtime = BotRuntime(
        "bot-1",
        {
            "run_id": "run-1",
            "worker_id": "worker-1",
            "canonical_fact_flush_interval_ms": 50,
            "shared_wallet_proxy": {
                "runtime_event_seq": _ProxySeqCounter(0),
                "lock": _ProxyLock(),
            },
            ...
            "facts": [
                {
                    "fact_type": "candle_upserted",
                    "series_key": "instrument-btc|1m",
                    "candle": {"time": "2026-04-20T05:20:00Z", "open": 1.0, "high": 1.5, "low": 0.9, "close": 1.1},
                }
            ],
        },
        batch_kind="botlens_runtime_bootstrap_facts",
    ...
    assert outcome.batch.seq == 1
    assert outcome.batch.append_result["retention_action"] == "transport_only"
    assert outcome.batch.append_result["queued"] is False
    assert appended_batches == []
...
def test_worker_runtime_treats_container_as_wallet_initialization_owner() -> None:
    runtime = BotRuntime(
        "bot-1",
        {
            "wallet_config": {"balances": {"USDC": 100}},
            "wallet_initialization_owner": "container",
            "shared_wallet_proxy": {
                "runtime_event_seq": _ProxySeqCounter(0),
                "runtime_events": [],
                "reservations": {},
    ...
    assert runtime._wallet_initialization_is_container_owned() is True
...
def test_run_artifact_payload_contains_runtime_event_stream_and_derived_views(monkeypatch):
    ...
    assert artifact["execution_mode"] == "full"
    assert artifact["playback_mode"] == "walk-forward"
    assert artifact["runtime_metadata"]["execution_mode"] == "full"
    assert artifact["runtime_metadata"]["intrabar_execution"] is True
...
def test_intrabar_fallback_runtime_event_serializes_context() -> None:
    bar_ts = datetime(2026, 1, 1, tzinfo=timezone.utc)
    event = new_runtime_event(
        ...
        context=ExecutionIntrabarFallbackContext(
            ...
            raw_reason="intrabar_missing",
            execution_mode="full",
            fallback_policy="pessimistic_stop",
            expected_intrabar_count=5,
            actual_intrabar_count=0,
        ),
        event_ts=bar_ts,
    )
    ...
    assert payload["event_name"] == "execution_intrabar_fallback_pessimistic"
    assert payload["context"]["reason"] == "missing_1m_data"
    assert payload["context"]["execution_mode"] == "full"
