...
def test_crossing_requires_explicit_transition_and_uses_canonical_streak() -> None:
    ...
    candles = [
        _candle(ts=_dt(1, 30), open_=101.0, close=101.0),  # no cross-up (already above)
        _candle(ts=_dt(2, 0), open_=101.0, close=102.0),   # still no cross-up
        _candle(ts=_dt(2, 30), open_=102.0, close=99.0),   # reset below
        _candle(ts=_dt(3, 0), open_=99.0, close=101.0),    # true crossed_up
        _candle(ts=_dt(3, 30), open_=101.0, close=102.0),  # streak=2 => emit
    ]
    outputs = _run_sequence(payload, candles)

    assert len(outputs[0]["signals"]) == 0
    assert len(outputs[1]["signals"]) == 0
    assert len(outputs[3]["signals"]) == 0
    assert len(outputs[4]["signals"]) == 1
    signal = outputs[4]["signals"][0]
...
def test_deterministic_single_profile_selected_per_bar() -> None:
    ...
    candles = [
        _candle(ts=_dt(1, 30), open_=104.0, close=109.0),
    ]
    outputs = _run_sequence(payload, candles)
...
def test_time_lockout_survives_state_reload() -> None:
    ...
    c1 = _candle(ts=_dt(1, 30), open_=99.0, close=101.0)   # emit
    ...
    c2 = _candle(ts=_dt(2, 0), open_=101.0, close=99.0)    # reset
    c3 = _candle(ts=_dt(2, 30), open_=99.0, close=101.0)   # +60m from first emit (locked out)
    c4 = _candle(ts=_dt(3, 0), open_=101.0, close=99.0)    # reset
    c5 = _candle(ts=_dt(3, 30), open_=99.0, close=101.0)   # +120m from first emit (allowed)
...
def test_timeframe_mismatch_diagnostic_is_emitted() -> None:
    ...
    assert int(diagnostics.get("source_timeframe_seconds") or 0) == 1800
    assert int(diagnostics.get("timeframe_mismatch_warning") or 0) == 1
