            ...
            if str(self.instrument_type or "").lower() == "spot":
                ...
                if not allowed:
                    if self.wallet_gateway:
                        self.wallet_gateway.reject(reason, payload)
                    ...
                    logger.warning(with_log_context("wallet_exit_rejected", context))
                    events.append(
                        {
                            "type": "execution_rejected",
                            "leg": leg.name,
            ...
            for leg in self.legs:
                ...
                if str(self.instrument_type or "").lower() == "spot":
                    ...
                    if not allowed:
                        ...
                        logger.warning(with_log_context("wallet_stop_rejected", context))
                        events.append(
                            {
                                "type": "execution_rejected",
                                "trade_id": self.trade_id,
...
class LadderRiskEngine:
    ...
    def _new_position(self, candle: Candle, direction: str) -> Optional[LadderPosition]:
        ...
        position = LadderPosition(
            ...
            execution_adapter=self.execution_adapter if self.instrument_type == "spot" else None,
            wallet_gateway=self._wallet_gateway if self.instrument_type == "spot" else None,
            base_currency=base_currency,
