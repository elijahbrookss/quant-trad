...
def detect_regime_changes(points: Sequence[Mapping[str, Any]]) -> List[int]:
    ...
    for entry in points:
        state = str(entry.get("regime_block_id") or entry.get("structure_state") or entry.get("state") or "").strip().lower()
...
def build_regime_markers(
    ...
    transition_epochs = []
    if block_points:
        blocks, _ = build_regime_blocks(block_points, timeframe_seconds=1, config=RegimeBlockConfig())
        for block in blocks:
            if (block.get("structure_state") or "").strip().lower() != "transition":
                continue
            epoch = _to_epoch(block.get("start_ts"))
            if epoch is not None:
                transition_epochs.append({"epoch": int(epoch), "confidence": block.get("avg_confidence")})
    ...
    logger.debug("regime_markers_built | points=%s | markers=%s", len(points), len(markers))
    return markers


def build_regime_marker_overlay(points: Sequence[Mapping[str, Any]], candles: Sequence[Candle]) -> Optional[Dict[str, Any]]:
    ...
    *,
    ...
    for entry in points:
        ...
        if not isinstance(candle_time, datetime):
            continue
        block_points.append({"time": candle_time, "structure_state": state, "confidence": entry.get("confidence")})
