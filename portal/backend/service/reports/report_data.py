...
def _runtime_decision_entry_from_event(row: Dict[str, Any]) -> Optional[Dict[str, Any]]:
    ...
    return {
        ...
        "signal_id": event_payload.get("signal_id") or event_payload.get("decision_id"),
        "source_type": event_payload.get("source_type") or ("runtime" if event_subtype in {"strategy_signal", "signal_accepted", "signal_rejected"} else None),
        "source_id": event_payload.get("source_id") or row.get("run_id"),
