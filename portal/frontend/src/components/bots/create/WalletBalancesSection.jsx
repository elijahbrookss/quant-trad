      ...
      <div className="space-y-2">
        <div className="space-y-1.5">
          {(walletBalances || []).map((row, index) => (
            <div key={row.id || `wallet-${index}`} className="flex items-center gap-1.5">
              <input
                type="text"
                value={row.currency}
                onChange={(event) => onWalletBalanceChange(index, { currency: event.target.value })}
                className="w-16 rounded-md border border-slate-800 bg-slate-950/50 px-2 py-1.5 text-[11px] font-medium uppercase text-slate-200 transition-colors focus:border-slate-700 focus:bg-slate-950 focus:outline-none"
                placeholder="USD"
              />
              <input
                type="number"
                ...
                className="min-w-[100px] flex-1 rounded-md border border-slate-800 bg-slate-950/50 px-2 py-1.5 text-[11px] tabular-nums text-slate-200 transition-colors focus:border-slate-700 focus:bg-slate-950 focus:outline-none"
                placeholder="10000"
              />
              {(walletBalances || []).length > 1 && (
                <button
                  ...
                  className="rounded-md p-1.5 text-slate-600 transition-colors hover:bg-rose-950/30 hover:text-rose-400"
                  aria-label="Remove"
                >
        ...
        >
          <PlusCircle className="size-3" /> Add currency
        </button>
        {walletError && (
          <div className="rounded-md border border-rose-900/50 bg-rose-950/20 px-2 py-1.5 text-[10px] text-rose-300">
            {walletError}
          </div>
        )}
      </div>
