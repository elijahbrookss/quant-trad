export function buildTriggerRows({ instrumentResult, rules = [], symbol = '' } = {}) {
  ...
  return rows
    .map((row, index) => {
      const rule = row?.rule_id ? ruleLookup.get(row.rule_id) : null
      ...
      return {
        id: row?.row_id || `${row?.rule_id || 'rule'}|${epoch || 0}|${index}`,
        rowKey: row?.row_id || `${row?.rule_id || 'rule'}|${epoch || 0}|${index}`,
        direction: side || (String(row?.action || '').toLowerCase() === 'buy' ? 'BUY' : 'SELL'),
        ruleId: row?.rule_id || null,
        ruleName: row?.rule_name || rule?.name || row?.rule_id || 'Rule',
        ...
        ruleRef: String(row?.rule_id || rule?.id || '').trim(),
